...
generated
`evidence/q286-wbss-k286-zero-residue-phase-pair-lift-drift-audit.json`, with
explanation in
...
Answer: no.  There are `18` violating rows across `14` distinct target
residues.  Four residues have two violating lifts.  Across those four repeated
residues, the top adverse pair is stable on only `1/4`, the top rescue pair on
`0/4`, the top absolute pair on `1/4`, and the `K_286` component sign on
`2/4`.  The repeated residues are `286`, `3718`, `4576`, and `7722`; only
`4576` keeps the same top adverse and top absolute pair, and no repeated
residue keeps the same top rescue pair.
...
`evidence/q286-wbss-k286-zero-residue-raw-budget-schedule.json`, with
explanation in
...
Answer: the zero lane is not worse than the global all-residue budget under
the crude residue-`L1` payment.  The total coefficient `L1` is
`372.962002076135`.  The global all-residue cap is
`0.0016192946592982506`, while the zero-lane caps range from
`0.0019144093398749697` to `0.0034092658993930512`.  The tightest zero-lane
residue is `9724 mod 10010`, with local main `0.7140019401930207`; the
loosest is `1716 mod 10010`, with local main `1.2715266354475274`.  The
tightest zero-lane cap is `1.1822489062642942` times looser than the global
cap.
...
`tools/build_q286_wbss_k286_zero_residue_raw_horizon_probe.py` generated
`evidence/q286-wbss-k286-zero-residue-raw-horizon-probe.json`, with
explanation in
...
Answer: no finite falsifier appears in this targeted probe.  The first `2`
lifts of all `35` period residues with `target_mod_286 = 0` survive:
`70/70` rows have positive raw witness, positive raw adverse-gate gap, and
`A_raw_-(N) < L_raw(N)`.  There are `0` zero total-weight rows.  The largest
raw adverse ratio is `0.22064948972651888`, and the tightest raw adverse gap
is `423543.19091507455`.  For comparison, the prior raw calibration's largest
ratio was `0.23148438379145228`, and its tightest raw gap was
`286929.1729900494`.
...
`tools/build_q286_wbss_k286_zero_residue_raw_coverage_audit.py` generated
`evidence/q286-wbss-k286-zero-residue-raw-coverage-audit.json`, with
explanation in
...
Answer: no.  Target residue `0 mod 286` is the unique combined two-mode
reflection no-discount residue at threshold `0.95`, with even energy fraction
`1.0` and odd energy fraction `1.8822507657823637e-28`.  The `348`-row raw
adverse-drag finite calibration samples `27` mod-286 residue classes but has
`0` rows with `target_mod_286 = 0`.  The finite raw stress classes are
different: highest raw adverse ratio occurs at `90 mod 286`
(`0.23148438379145228`), and tightest `gap/N` occurs at `134 mod 286`
(`0.26310340793692394`).
...
`evidence/q286-wbss-k286-two-mode-singular-target.json`, with explanation in
...
Answer: in principal-normalized coordinates,
...
Using the previous aggregate cap `||delta||_2 <= 0.038004233097145394`, the
residual singular tail would cost at most `0.02722757770861337`, leaving the
sufficient two-mode floor

```text
Re(sigma_1*mu_1(N)+sigma_2*mu_2(N)) >= -0.14867615917424493.
```

Decision: `TARGET_k286_two_mode_singular_target_unproved`.  The next
proof-shaped target is now exact: prove a one-sided lower bound for the two
singular moment functionals, plus a residual cap, instead of paying all active
`K_286` characters independently.  This proves no twisted binary-prime moment
...
in `notes/q286-wbss-centered-support-inequality-ledger.md`.
...
same raw major/minor arc proof.  For support bucket `B`, prove
...
The ledger fixes one explicit sufficient budget:
...
dominant buckets:                 286, 154, 70
tail buckets:                     10, 14, 22, 26, 130
```
...
explanation in `notes/q286-wbss-single-weight-major-arc-floor-audit.md`.
...
ranges from `-11.747067126163136` to `40.228810300155935`.  It has `1228`
negative unit residues, `0` zero unit residues, and `1652` positive unit
residues.  Every one of the `5005` even target residues has at least one
admissible negative-weight left-prime residue, and no target residue has a
pointwise positive floor.

The local-uniform averages remain positive: the weakest mean is
`0.6039353780830684`, and the largest negative admissible fraction is
`0.47205387205387206`.  This means the local-uniform major term survives, but
positivity depends on distribution of prime-pair mass, not merely on existence
of a strict-central prime pair.

Decision: `FALSIFIER_pointwise_positive_single_weight_floor`.  The support-only
shortcut is dead.  The surviving theorem target must prove a universal
pointwise signed distribution estimate, a one-sided adverse-drag envelope below
local main, or a direct major/minor arc estimate for `W_phi(N)>0`.  This proves
no signed distribution theorem, adverse-drag theorem, major/minor arc estimate,
...
`tools/build_q286_wbss_strict_raw_gap_analytic_hold.py` generated
`evidence/q286-wbss-strict-raw-gap-analytic-hold.json`, with explanation in
`notes/q286-wbss-strict-raw-gap-analytic-hold.md`.

Question: finite evidence is no longer an acceptance condition.  After the
strict raw-gap correction, what analytical estimate would actually prove the
bridge?

Answer: the live non-circular bridge is still
...
Sufficient analytic routes are now explicit:
...
B is also sufficient, but its positive-mass input is already strict-central
existence strength.
...
Decision: `HOLD_for_strict_raw_binary_pair_estimate`.  The repository currently
has no theorem proving the needed direct signed raw sum, positive mass plus
one-sided projection control, or fixed-modulus binary-prime convolution bound.
Rejected acceptance substitutes are: more finite q286 rows, fitted residual
absorption constants such as `.125`, `.126`, or `.13`, normalized `L2` or `L1`
discrepancy alone, one-dimensional AP prime estimates alone, and almost-all or
averaged estimates with unnamed exceptions.  No direct raw signed binary-prime
theorem, positive-mass theorem, one-sided projection theorem, q286 threshold
theorem, strict-central Goldbach theorem, or Goldbach proof is established.
...
in `notes/q286-direct-witness-sign-split-budget-audit.md`.

Question: after anti-landing proved near-sharp, can the project bypass that
edge construction and prove direct `B_Phi(N)>0` by splitting the raw
coefficient into positive and negative cells?

Answer: no, not as an easier route.  On the same `196` post-discovery rows:
...
Decision: `HOLD_direct_sign_split_not_escape_from_sharp_correlation`.  The
full raw direct split is even tighter than anti-landing, and the edge-beta
split reproduces anti-landing.  A naive positive-versus-negative coefficient
landing theorem is therefore not an easier route to direct `B_Phi(N)>0`.
The next proof object must be a more adapted signed binary-prime correlation
estimate, not broad `L1`, mass-majority, AP counts, or raw sign-split
landing.  This proves no direct witness theorem, signed binary-prime
correlation theorem, q286 threshold theorem, strict-central Goldbach theorem,
or Goldbach proof.
...
audit:
`tools/build_q286_anti_landing_source_budget_audit.py` generated
`evidence/q286-anti-landing-source-budget-audit.json`, with explanation in
`notes/q286-anti-landing-source-budget-audit.md`.

Question: if a future source-backed binary-prime theorem tries to prove the
coefficient-weighted anti-landing inequality
`positive_rescue_contribution > negative_rescue_drag`, how sharp must it be?

Answer: near-sharp.  On the `196` post-discovery rows, the tightest row is
target `94856`:
...
Decision: `HOLD_anti_landing_requires_near_sharp_source_control`.  Broad
uniformity, mass-majority, and loose AP-count inputs cannot pay this bridge.
A proof following this route needs a sharp coefficient-weighted signed
binary-prime landing estimate, or the project should pivot to a different
direct witness argument.  This proves no anti-landing theorem, signed
...
in `notes/q286-active-selector-l2-bridge-status-audit.md`.

Question: is the active-selector `L2` rarity target genuinely non-circular,
and is it actually confirmed?

Answer: `L2` smallness is a valid non-circular sufficient theorem shape if
proved externally as a pointwise arithmetic estimate for actual
strict-central binary-prime residue weights with `T_N>0`.  It is not
confirmed by current work, and it cannot by itself prove existence because it
is normalized by `T_N`.
...
Decision: `HOLD_L2_is_valid_sufficient_non_circular_shape_but_unconfirmed`.
The theorem-shaped next step should prefer the sharper signed-projection
obligation `<delta_N,c_a> >= -0.3`, or the equivalent residue-pair
correlation inequality, over plain generic `L2` uniformity.  No `L2`
discrepancy theorem, active-selector rarity theorem, strict-central
...
Question: does the aggregate character `L2` cap already hold on the checked
rows, and are any failures caused by zero strict-central mass?

Answer: no, the cap does not already hold; and no, the failures are not a
zero-mass normalization artifact.
...
`L2` formulation remains useful as a possible asymptotic theorem shape, but
the checked population falsifies any claim that the displayed cap already
holds at this scale.  The zero-mass check passes, so the finite failures are
real finite arithmetic obstructions rather than missing-mass artifacts.  The
route now requires an explicit threshold beyond the observed violations, a
stronger structured signed moment theorem than plain aggregate `L2`, or a
different signed estimate.  This confirms no non-circular analytic bridge,
aggregate character moment theorem, pointwise adverse-drag theorem, q286
threshold theorem, strict-central Goldbach theorem, or Goldbach proof.
...
Question: after the four-modulus span identity, can Fourier/projection
sparsification produce a smaller coefficient theorem target than
`{70,130,154,286}`?

Result: all `16` subfamilies of `{70,130,154,286}` were enumerated at the
unit-coefficient span level. The full four-modulus family has relative L2
residual `8.75253864513401e-16` and maximum absolute residual
`2.1316282072803e-14`. No proper subfamily is exact. The best proper family is
`{70,154,286}`, with relative L2 residual `0.0252841747236269` and maximum
absolute residual `0.273926119559664`.

Decision: Fourier/projection sparsification does not produce a smaller
coefficient theorem target at this algebraic level. The narrowed live target
...
eventually positive. This proves no universal projection-control theorem,
...
in `notes/q286-wbss-four-modulus-span-identity-audit.md`.

Question: is the prior `[70,130,154,286]` closure just another stress-sample
renaming of the hard signed expectation, or is `130` a genuine missing
projection coordinate?

Result: the stronger unit-level identity passes.  The full q286-WBSS unit
coefficient on units modulo `10010` lies in the additive row span generated by
total mass plus projections to moduli `70,130,154,286`, with relative residual
`8.752538645134008e-16`.  Dominant-only supports `[70,154,286]` fail at the
unit level with relative residual `0.025284174723626925`.  Since unit-level
descent implies the reflected target-orbit coefficient identity, the
four-modulus coefficient span covers all `5005` even target residues modulo
`10010`.  Seven representative target residues replayed directly all passed
the four-modulus span check and all failed under dominant-only supports.

Decision: this is a genuine finite algebraic compression, not merely a new
name for the stress LP.  The hole still is not closed.  The remaining theorem
target is strict-central binary-prime projection control for moduli
`70,130,154,286`, or an equivalent direct raw signed witness estimate.
...
`0.546820393849208`, and `0.48379401372791037`.  The channel-margin
contribution dominates the driver-margin contribution in all three rows,
which points the next theorem pressure toward fixed-conductor channel slack.
...
cherry-picking risk remain open; the next non-circular test is either a
larger predeclared selector holdout on new windows or a strict-closure stress
pass only if an unchanged selector holdout actually produces active rows.
...
The first actual visual superposition is now generated as
`evidence/q286-visual-summary.svg`, produced by
`tools/build_q286_visual_summary_svg.py`.  It overlays selected component PCA,
alignment/complement PCA, role shapes, glow-scaled target sizes,
`(5,7)/(7,11)` component-vector arrows, and mechanism/theorem glow bars.  It
also preserves the intended order of evidence: finite selectors and receipts
feed vector coordinates, vector coordinates feed PCA/shape views, and the
...
### 2026-09-14 continuation: dominant-mode branch holdout

The signed-channel branch split for the first two dominant q286 modes was
applied unchanged to the next nonoverlapping deterministic holdout window
after the recorded `1242000` stress neighborhood.  The new receipt and
evidence are:
...
The tightest clear row is `1243130`, with positive offset slack about
`0.0340510877`.  The maximum-pressure clear row is `1243018`, with negative
pressure about `0.2586239309`, still below the `0.3` pressure branch
threshold.

This is finite branch-denominator evidence only.  It strengthens the observed
post-`1242000` branch posture but does not prove the eventual branch theorem,
the pointwise character-sum estimate, the signed-projection theorem, or
