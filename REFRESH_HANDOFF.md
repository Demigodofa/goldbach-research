...
## 2026-09-12: reduced-envelope mode signatures inspected
...
The first-cycle worst targets remain mode-1 dominated: `10424` has q286
deviation `-0.970542065173128` principal, six-mode sum
`-0.9618075775131865`, and mode contributions
`[-0.570461,-0.304579,-0.066155,-0.035986,0.031383,-0.016011]`; `14138` has
deviation `-0.8666741397111133`, six-mode sum `-0.8667122522823336`, and
mode contributions `[-0.738353,-0.13155,-0.025111,0.040189,-0.013751,0.001864]`.

The later shallow negatives are still mostly a modes-1/2 phenomenon but not a
fixed-sign theorem.  At `30164`, mode `2` is slightly larger than mode `1`;
at `40676`, mode `1` leads but modes `1` and `2` together carry almost all
negative mass.  The reduced-model-only boundary target `11194` has positive
mode `1` but large negative mode `2`, while `15272` is tail-rescued by a
positive q286 residual of `0.018953225776954182` principal.
...
## 2026-09-12: first-two q286 modes isolate sampled negatives
...
obstruction.  It compares each target's full-action ratio with the same ratio
after removing modes `1` and `2`, and records first-two capture on negative
full-action targets.

Default scan: four `501`-target windows covering the same ranges as the
reduced-envelope cycle scanner.  All `19` negative full-action targets have
negative first-two q286 mode sum.  The first-two negative contribution is at
least `1.36990224676328` times the full negative deficit on those targets,
and can be much larger near the sign boundary.  Removing modes `1` and `2`
leaves a positive lower envelope on the sampled set: the worst full action
without those modes is `0.09048725420345835` principal at `N=10354`; the
worst reduced action without those modes is `0.08579199551280053` there.
The most negative first-two mode sum is `-1.1124437977839818` principal at
`N=10664`.
...
the minimum first-three capture is `1.02060949158308`.  The most negative
first-two and first-three sums both occur at `N=10664`, with ratios
`-1.1124437977839818` and `-1.1500880008976306`.
...
-0.195348, 0.093594`.  For `10664`: `-1.150088, -0.306513, -0.148512,
...
The explicit `.63/.3/.47` candidate was tested on fresh cycles `57..64`.
Every cycle passed; cycle `61` had no `.3` tail targets.  No complement-floor,
rescue-margin-floor, or deficit-ceiling violations occurred.  The limiting
nonempty values are margin `0.4482571418757897` at cycle `58` and deficit
`0.4144257505601422` at cycle `64`.
...
Using the fast mode-only scanner, global cycles `233..264` were scanned in
four 8-cycle workers.  Every cycle was tail-free at threshold `.3`; no
rescue-floor check was needed.
...
```text
233..240: cycle 235, target 2366548, first_three -0.18648556964961702
241..248: cycle 245, target 2466634, first_three -0.18814240147876807
249..256: cycle 250, target 2516512, first_three -0.1956485325659766
257..264: cycle 257, target 2584486, first_three -0.18282739060588282
```

Current interpretation: sparse selected-tail recurrences were seen through
cycle `136`; checked cycles `137..264` are tail-free.  This is still finite
evidence only and does not prove eventual disappearance.

## 2026-09-13 continuation: cycles 265 through 296 remain tail-free

Using the fast mode-only scanner, global cycles `265..296` were scanned in
four 8-cycle workers.  Every cycle was tail-free at threshold `.3`; no
rescue-floor check was needed.
...
```text
265..272: cycle 266, target 2672932, first_three -0.19902872242009065
273..280: cycle 273, target 2751106, first_three -0.1771080171549704
281..288: cycle 288, target 2900782, first_three -0.16965353530921884
289..296: cycle 292, target 2935984, first_three -0.19368388953654794
```
...
`119.854s`.  It rewrites the selected lower-support rescue condition as a
...
Over the late comparison set, the uniform selected sufficient floor is
...
in `121.972s`.  It splits floor stability into a selected required
lower-support package ceiling
`required_lower_support_package_to_rescue <= -0.6751407665271594` and a
selected fixed-offset floor
`non_pair_lower_support_actual + component_pair_local_mean >=
0.2231338491453711`.  The three late comparison targets satisfy both term
