...
This turns the four-modulus target from a stress-LP observation into a finite
coefficient-algebra identity.  The full q286-WBSS unit coefficient modulo
`10010` lies in the additive row span generated by total mass and projections
to:
...
with relative residual `8.752538645134008e-16`.  The dominant-only span
`[70,154,286]` fails with relative residual `0.025284174723626925`.

Because unit-level descent implies the corresponding reflected target-orbit
span identity, the four-modulus coefficient compression covers all `5005`
even target residues modulo `10010`.

This is a real narrowing of the coefficient side.  It still does not supply
the prime-distribution side: the remaining source-backed theorem must control
the actual strict-central binary-prime projections to `70,130,154,286`, or
prove `B_Phi(N)>0` directly.
