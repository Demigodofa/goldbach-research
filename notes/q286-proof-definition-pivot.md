...
## Four-modulus coefficient identity
...
The minimal tail repair is not merely a fitted stress-row clause.  The full
q286-WBSS unit coefficient on units modulo `10010` descends to the additive
projection span generated by total mass and moduli:

```text
70, 130, 154, 286
```

The unit-level relative residual is `8.752538645134008e-16`.  The
dominant-only supports `[70,154,286]` still fail with residual
`0.025284174723626925`.  Unit-level descent implies the same four-modulus
span identity for every reflected target-orbit coefficient, hence for all
`5005` even target residues modulo `10010`.

This changes the proof-definition pivot slightly.  The missing object is no
longer "some lower-modulus cone" or "all coefficient supports"; it is the
specific four-modulus projection theorem:

```text
actual strict-central binary-prime measures have enough controlled projection
data modulo 70, 130, 154, and 286 to force <mu_N,Phi_a> > 0.
```

That theorem is still unproved, and proving it may still be as hard as the
original signed prime-correlation problem.
