"""Build the full M=251 prime-row source-start sweep.

M=229 was the first complete fresh-scale source-start sweep beyond the
original fixture.  This audit repeats the full-prime-row check at the next
fresh scale, M=251.  It remains finite evidence, not an asymptotic theorem.
"""
...
def build_receipt():
    ...
    return {
        ...
        "status": "SWEEP_degree5_source_start_m251_full_prime_rows",
        "question": (
            "After the full M=229 source-start sweep survived, do all "
            "canonical source-start prime rows for the next fresh scale, "
            "M=251, satisfy signed active/full dominance?"),
        "scale_modulus": SCALE_MODULUS,
        ...
        "strongest_dominance_row": strongest,
        "prediction": (
            "If the narrowed source-start lane is stable across adjacent "
            "fresh scales, every prime row in [251, 502] should retain "
            "positive signed active/full slack."),
        "falsifier": (
            "Any M=251 prime-row component or total row with negative full "
...
def main():
    ...
    if receipt["all_rows_dominate_one_half_signed_full"]:
        receipt["decision"] = (
            "The full M=251 prime-row source-start sweep survives: every "
            "prime row in [251, 502] has positive active/full dominance slack "
            "above one half.  This is one complete fresh scale only and "
            "proves no universal theorem.")
    else:
        receipt["decision"] = (
            "The full M=251 prime-row source-start sweep found at least one "
            "signed active/full failure.  This is a finite falsifier for the "
            "M=251 full fresh-scale source-start sweep.")
