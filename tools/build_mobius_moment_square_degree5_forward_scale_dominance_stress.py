"""Build the forward-scale degree-5 primewise dominance stress audit."""
...
def summarize_rows(scale_modulus):
...
def build_receipt():
    scale_summaries = [
        summarize_rows(scale_modulus)
        for scale_modulus in FORWARD_SCALES
    ]
    all_slacks = []
    all_failures = []
    for summary in scale_summaries:
        all_slacks.extend([
            ...
            summary["maximum_dominance_slack_above_one_half"],
        ])
    ...
    return {
        ...
        "question": (
            "Does the checked-scale degree-5 active/full dominance pattern "
            "survive the next adjacent prime scale modulus after 227, or "
            "does this forward stress fixture find a concrete falsifier?"),
        ...
        "prediction": (
            "If the local one-prime degree-5 active/full dominance target "
            "is robust beyond the six-scale Sturm fixture, the adjacent "
            "forward prime scale moduli should retain positive slack above "
            "one half."),
        "falsifier": (
            "Any component or degree-5 total row in these forward scales "
            "with negative full contribution and active/full ratio at or "
            "below one half is a concrete finite falsifier for this "
            "forward-stress extension."),
        "decision": (
            "The adjacent forward-scale stress fixture survives: every "
            "checked component and degree-5 total row at scale modulus 229 "
            "retains positive active/full dominance slack above one half.  "
            "This extends the finite dominance evidence one scale beyond the "
            "checked Sturm fixture, but it is still not a theorem."),
        "finite_forward_scale_dominance_stress_only": True,
        "finite_diagnostic_only": True,
        "goldbach_proved": False,
        "q286_reactivated": False,
        "forward_scale_dominance_theorem_proved": False,
