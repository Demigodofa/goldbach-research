"""Build the weak-scale degree-5 ell-freeze perturbation stress audit."""

import json
import sys
from pathlib import Path

import numpy as np

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from lcm_sawtooth_lifted_endpoint_frame import (  # noqa: E402
    lifted_endpoint_residue_gram_receipt,
)
from mobius_covariance_endpoint_probe import _prime_flags  # noqa: E402
from tools.build_mobius_moment_square_degree5_checked_scale_dominance_audit import (  # noqa: E402
    PAIRS,
    THRESHOLD,
    dominance_row,
    scale_parameters,
    sign_counts,
)


WEAK_SCALE = 167
ELL_OFFSETS = (-2, -1, 0, 1, 2)
OUTPUT = Path(
    "evidence/mobius-moment-square-degree5-ell-freeze-perturbation-stress.json")


def ell_freeze_values():
    base = scale_parameters(WEAK_SCALE)["ell_freeze"]
    return [base + offset for offset in ELL_OFFSETS]


def summarize_ell(ell_freeze):
    parameters = scale_parameters(WEAK_SCALE)
    divisor_lower, divisor_upper = parameters["divisor_range"]
    flags = _prime_flags(2 * WEAK_SCALE)
    dominance_rows = []
    failure_rows = []
    for prime_modulus in range(WEAK_SCALE, 2 * WEAK_SCALE + 1):
        if not flags[prime_modulus]:
            continue
        receipt = lifted_endpoint_residue_gram_receipt(
            prime_modulus,
            parameters["row_count"],
            ell_freeze,
            divisor_lower,
            divisor_upper,
        )
        active = np.asarray(receipt["active_window_residue_energy_gram"])
        active = (active + active.T) / 2
        full = np.asarray(receipt["full_residue_energy_gram"])
        full = (full + full.T) / 2
        component_rows = []
        for left, right, left_label, right_label in PAIRS:
            active_contribution = float(2 * active[left, right])
            full_contribution = float(2 * full[left, right])
            half_contribution = (
                active_contribution - THRESHOLD * full_contribution)
            row = dominance_row(
                WEAK_SCALE,
                prime_modulus,
                f"{left_label},{right_label}",
                active_contribution,
                full_contribution,
                half_contribution,
            )
            row["ell_freeze"] = ell_freeze
            row["ell_offset"] = ell_freeze - parameters["ell_freeze"]
            component_rows.append(row)
            dominance_rows.append(row)
        active_total = sum(
            row["active_contribution"] for row in component_rows)
        full_total = sum(
            row["full_contribution"] for row in component_rows)
        half_total = sum(
            row["half_frame_contribution"] for row in component_rows)
        total_row = dominance_row(
            WEAK_SCALE,
            prime_modulus,
            "TOTAL",
            active_total,
            full_total,
            half_total,
        )
        total_row["ell_freeze"] = ell_freeze
        total_row["ell_offset"] = ell_freeze - parameters["ell_freeze"]
        dominance_rows.append(total_row)
        for row in [*component_rows, total_row]:
            if not row["dominates_one_half_signed_full"]:
                failure_rows.append(row)
    slacks = [
        row["dominance_slack_above_one_half"]
        for row in dominance_rows
    ]
    return {
        "scale_modulus": WEAK_SCALE,
        "ell_freeze": ell_freeze,
        "ell_offset": ell_freeze - parameters["ell_freeze"],
        "row_count": parameters["row_count"],
        "base_ell_freeze": parameters["ell_freeze"],
        "divisor_range": parameters["divisor_range"],
        "prime_count": len({
            row["prime_modulus"] for row in dominance_rows
        }),
        "dominance_row_count": len(dominance_rows),
        "component_row_count": sum(
            1 for row in dominance_rows if row["label"] != "TOTAL"),
        "degree5_total_row_count": sum(
            1 for row in dominance_rows if row["label"] == "TOTAL"),
        "dominance_slack_sign_counts": sign_counts(slacks),
        "all_rows_dominate_one_half_signed_full": all(
            row["dominates_one_half_signed_full"]
            for row in dominance_rows),
        "minimum_dominance_slack_above_one_half": min(slacks),
        "maximum_dominance_slack_above_one_half": max(slacks),
        "weakest_dominance_row": min(
            dominance_rows,
            key=lambda row: row["dominance_slack_above_one_half"]),
        "strongest_dominance_row": max(
            dominance_rows,
            key=lambda row: row["dominance_slack_above_one_half"]),
        "failure_rows": failure_rows,
    }


def build_receipt():
    base_parameters = scale_parameters(WEAK_SCALE)
    ell_summaries = [
        summarize_ell(ell_freeze)
        for ell_freeze in ell_freeze_values()
    ]
    all_failures = []
    for summary in ell_summaries:
        all_failures.extend(summary["failure_rows"])
    weakest = min(
        (summary["weakest_dominance_row"] for summary in ell_summaries),
        key=lambda row: row["dominance_slack_above_one_half"])
    strongest = max(
        (summary["strongest_dominance_row"] for summary in ell_summaries),
        key=lambda row: row["dominance_slack_above_one_half"])
    row_count = sum(
        summary["dominance_row_count"] for summary in ell_summaries)
    component_count = sum(
        summary["component_row_count"] for summary in ell_summaries)
    total_count = sum(
        summary["degree5_total_row_count"] for summary in ell_summaries)
    slack_sign_counts = {
        "positive": row_count - len(all_failures),
        "zero": sum(
            1 for row in all_failures
            if row["dominance_slack_above_one_half"] == 0),
        "negative": sum(
            1 for row in all_failures
            if row["dominance_slack_above_one_half"] < 0),
    }
    ell_values = ell_freeze_values()
    return {
        "status": "STRESS_degree5_ell_freeze_perturbation_dominance",
        "question": (
            "Does the weak-block degree-5 active/full dominance target depend "
            "delicately on the exact fitted logarithmic ell_freeze=52, or "
            "does it survive a small local perturbation of that coefficient "
            "freeze?"),
        "scale_modulus": WEAK_SCALE,
        "threshold": THRESHOLD,
        "base_parameters": {
            "row_count": base_parameters["row_count"],
            "ell_freeze": base_parameters["ell_freeze"],
            "divisor_range": base_parameters["divisor_range"],
        },
        "ell_offsets": ELL_OFFSETS,
        "ell_freezes": ell_values,
        "ell_summaries": ell_summaries,
        "prime_count_per_ell": ell_summaries[0]["prime_count"],
        "ell_count": len(ell_summaries),
        "component_row_count": component_count,
        "degree5_total_row_count": total_count,
        "dominance_row_count": row_count,
        "all_dominance_slack_sign_counts": slack_sign_counts,
        "all_rows_dominate_one_half_signed_full": len(all_failures) == 0,
        "failure_row_count": len(all_failures),
        "failure_rows": all_failures,
        "minimum_dominance_slack_above_one_half": (
            weakest["dominance_slack_above_one_half"]),
        "maximum_dominance_slack_above_one_half": (
            strongest["dominance_slack_above_one_half"]),
        "weakest_dominance_row": weakest,
        "strongest_dominance_row": strongest,
        "prediction": (
            "If the weak-block one-prime degree-5 dominance phenomenon is not "
            "merely tuned to the source logarithmic ell_freeze, nearby "
            "ell_freeze values should retain positive slack above one half "
            "for the same component and total rows."),
        "falsifier": (
            "Any checked component or degree-5 total row for ell_freeze 50 "
            "through 54 whose signed full contribution is negative but whose "
            "active/full ratio is at or below one half, or any row losing the "
            "negative signed-full premise, is a concrete finite falsifier for "
            "this local perturbation stress."),
        "decision": (
            "The weak-scale ell-freeze perturbation stress survives on the "
            "checked window: every component and degree-5 total row for "
            "ell_freeze 50 through 54 retains positive active/full dominance "
            "slack above one half.  This reduces concern that the finite "
            "weak-block result is only an exact logarithmic ell_freeze=52 "
            "fit, but it remains finite perturbation evidence rather than a "
            "theorem."),
        "finite_ell_freeze_perturbation_stress_only": True,
        "finite_diagnostic_only": True,
        "goldbach_proved": False,
        "q286_reactivated": False,
        "ell_freeze_dominance_theorem_proved": False,
        "checked_scale_dominance_theorem_proved": False,
        "primewise_dominance_theorem_proved": False,
        "degree5_coefficient_theorem_proved": False,
        "robust_margin_universal_theorem_proved": False,
        "coefficient_family_theorem_proved": False,
        "universal_sturm_certificate_theorem_proved": False,
        "moment_square_half_frame_curve_positivity_theorem_proved": False,
        "uniform_active_full_lower_frame_proved": False,
        "mobius_covariance_theorem_proved": False,
        "signed_prime_correlation_proved": False,
    }


def main():
    receipt = build_receipt()
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(
        json.dumps(receipt, indent=2, sort_keys=True) + "\n",
        encoding="utf-8")
    print(f"wrote {OUTPUT}")


if __name__ == "__main__":
    main()
