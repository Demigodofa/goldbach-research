"""Build the full M=383 prime-row source-start sweep with resume support."""
...
CHECKPOINT = (
    ROOT
    / "evidence"
    / "mobius-moment-square-degree5-source-start-m383-full-prime-sweep.partial.json")
...
def checkpoint_payload(prime_rows, run_elapsed_seconds):
    expected = m383_prime_moduli()
    sorted_rows = sorted(prime_rows, key=lambda row: row["ordinal"])
    completed = [row["prime_modulus"] for row in sorted_rows]
    completed_set = set(completed)
    pending = [prime for prime in expected if prime not in completed_set]
    return {
        "schema_version": 1,
        "source_commit": source_commit(),
        "status": "PARTIAL_degree5_source_start_m383_full_prime_rows",
        ...
        "completed_prime_rows": sorted_rows,
...
def summarize_full_sweep(prime_rows, total_elapsed_seconds):
    ...
    weakest = min(
        all_rows, key=lambda row: row["dominance_slack_above_one_half"])
    strongest = max(
        all_rows, key=lambda row: row["dominance_slack_above_one_half"])
    ...
    receipt = {
        ...
        "question": (
            "Does the next complete source-start sweep M=383 preserve signed "
            "active/full dominance, coherent prime-block stress morphology, "
            "and the clearance-family denominator target?"),
        "scale_modulus": SCALE_MODULUS,
        "prime_interval": [SCALE_MODULUS, 2 * SCALE_MODULUS],
        ...
        "top_four_prime_modulus": blocks[0]["prime_modulus"],
        "top_four_labels": [row["label"] for row in top_four_rows],
        ...
        "prediction": (
            "If prime-block morphology persists beyond M=379, the M=383 "
            "tightest rows should come from one prime block containing the "
            "three tracked components and TOTAL."),
        "falsifier": (
            "Positive dominance can survive while morphology fails: the finite "
            "morphology is falsified if the tightest rows split across "
    ...
    if receipt["all_rows_dominate_one_half_signed_full"]:
        if receipt["top_four_rows_form_single_prime_block"]:
            ...
            morphology_sentence = (
                ...
                "prime-block morphology is falsified for M=383.")
        receipt["decision"] = (
            "The full M=383 prime-row source-start sweep survives signed "
            f"dominance.  {morphology_sentence} This is one complete fresh "
            "scale only and proves no universal theorem.")
    else:
        receipt["decision"] = (
            "The full M=383 prime-row source-start sweep found at least one "
            "signed active/full failure.  This is a finite falsifier for the "
            "M=383 full fresh-scale source-start sweep.")
    return receipt
...
def build_receipt():
    ...
    pending = [
        ...
        if prime not in completed
    ]
    if pending:
        print(
            f"computing {len(pending)} pending M={SCALE_MODULUS} rows with "
            f"{MAX_WORKERS} workers",
            flush=True,
        )
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(timed_source_start_prime_row, ordinal, prime): prime
            for ordinal, prime in pending
        }
        for future in as_completed(futures):
            row = future.result()
            prime_rows.append(row)
            completed.add(row["prime_modulus"])
            write_json_atomic(
                CHECKPOINT,
                checkpoint_payload(prime_rows, perf_counter() - start))
    prime_rows = sorted(prime_rows, key=lambda row: row["ordinal"])
    if CHECKPOINT.exists():
        CHECKPOINT.unlink()
    return summarize_full_sweep(prime_rows, perf_counter() - start)
