"""Build the full M=379 prime-row source-start sweep with resume support.

The five-scale morphology audit says the next finite falsifier is a complete
fresh sweep whose tightest rows split across unrelated primes or isolate one
component far below its prime companions.  This runner tests the next scale,
M=379, against that stated prime-block morphology target.
"""
...
CHECKPOINT = (
    ROOT
    / "evidence"
    / "mobius-moment-square-degree5-source-start-m379-full-prime-sweep.partial.json")
...
def checkpoint_payload(prime_rows, run_elapsed_seconds):
    ...
    return {
        ...
        "status": "PARTIAL_degree5_source_start_m379_full_prime_rows",
...
def summarize_full_sweep(prime_rows, total_elapsed_seconds):
    ...
    return {
        ...
        "question": (
            "Does the next complete source-start sweep M=379 preserve signed "
            "active/full dominance and the prime-block stress morphology seen "
            "through M=353?"),
        "scale_modulus": SCALE_MODULUS,
        "prime_interval": [SCALE_MODULUS, 2 * SCALE_MODULUS],
        ...
        "prediction": (
            ...
            "M=379 should come from one prime block containing the three "
    ...
    if receipt["all_rows_dominate_one_half_signed_full"]:
        if receipt["top_four_rows_form_single_prime_block"]:
            ...
            morphology_sentence = (
                ...
                "prime-block morphology is falsified for M=379.")
        receipt["decision"] = (
            "The full M=379 prime-row source-start sweep survives signed "
            f"dominance.  {morphology_sentence} This is one complete fresh "
            "scale only and proves no universal theorem.")
    else:
        receipt["decision"] = (
            "The full M=379 prime-row source-start sweep found at least one "
            "signed active/full failure.  This is a finite falsifier for the "
            "M=379 full fresh-scale source-start sweep.")
    return receipt
