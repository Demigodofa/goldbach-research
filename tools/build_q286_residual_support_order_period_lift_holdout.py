"""Period-lift holdout for the q286 support-order sign bridge.

The support-order sign bridge survived the original seven-row fixture.  This
receipt applies the same support-size <= 2 split to consecutive forward
period-lifts of those seven residues, without changing the mask or refitting a
tail envelope.

Finite holdout audit only.  It proves no eventual-threshold theorem, low-order
base theorem, high-order tail domination theorem, q286 threshold theorem,
...
SOURCE = EVIDENCE / "q286-residual-support-order-sign-bridge-audit.json"
OUT = EVIDENCE / "q286-residual-support-order-period-lift-holdout.json"
LIFT_COUNT = 8
TOLERANCE = 1e-10
...
def build_receipt():
    ...
    return {
        ...
        "sources": {
            "support_order_sign_bridge": str(SOURCE.relative_to(ROOT)),
            "support_order_sign_bridge_status": source["status"],
        },
        "status": "CANDIDATE_period_lift_support_order_bridge_survives_holdout",
        "status_boundary": (
            "finite period-lift holdout only; no eventual-threshold theorem, "
        ...
        "candidate": {
            "name": "period-lift support-order threshold scout",
            "novelty_label": "new-to-this-task",
            "mechanism": (
                "Keep the exact support-size <= 2 split from the fixture and "
                "evaluate consecutive forward period-lifts of the same seven "
                "residues.  This changes the prime-pair masses while "
                "preserving the support-order dictionary and residue operator."),
            "prediction": (
                "If the support-order split is an eventual theorem target, "
                "forward period-lifts should retain low-order base positivity "
                "and high-order tail domination with more slack than the early "
                "tight fixture row."),
            "falsifier": (
                "A lifted row with positive full action but nonpositive "
                "low-order base, or an adverse high-order tail at least as "
                "large as the low-order base, breaks the support-order bridge "
                "as an eventual-threshold candidate on this holdout."),
            "smallest_test": (
                "Evaluate the first eight forward period-lifts of the seven "
                "original support-packet residues."),
        },
        "period": PERIOD,
        ...
        "summary": {
            ...
            "holdout_high_order_adverse_envelope": (
                high_order_adverse_envelope),
            "holdout_envelope_margin_at_tight_base": (
                envelope_margin_at_tight_base),
            "maximum_reconstruction_error": max(
                abs(row["signed_reconstruction_error"]) for row in rows),
        },
        "decision": (
            "The support-order bridge survives the first eight forward "
            "period-lifts of the original seven residues.  All 56 holdout "
            "rows are full-positive and low-order-base-positive, with no "
            "sign mismatches.  The tight low-order base is about 0.37397 at "
            "target 124886, much larger than the original tight base near "
            "0.05371.  The worst adverse high-order tail consumes about "
            "13.27% of its base on target 164926, far below the original "
            "76.58% fixture stress.  On this lifted holdout only, even the "
            "disconnected high-order adverse envelope stays below every "
            "low-order base.  This supports an eventual-threshold hypothesis "
            "for the support-order split, but proves no threshold theorem and "
            "does not supply a non-circular analytic estimate.  Goldbach "
            "remains open."),
    }
