import json
import unittest
from pathlib import Path

from tools.build_mobius_moment_square_degree5_weak_scale_phase_curve_sweep import (
    active_row_start_sweep,
)


EVIDENCE = Path(
    "evidence/mobius-moment-square-degree5-weak-scale-phase-curve-sweep.json")


class MobiusMomentSquareDegree5WeakScalePhaseCurveSweepTests(
        unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.receipt = json.loads(EVIDENCE.read_text(encoding="utf-8"))

    def test_sweep_covers_weak_scale_primes_and_local_row_band(self):
        self.assertEqual(active_row_start_sweep(), list(range(71)))
        self.assertEqual(self.receipt["scale_modulus"], 167)
        self.assertEqual(self.receipt["row_count"], 35)
        self.assertEqual(self.receipt["ell_freeze"], 52)
        self.assertEqual(self.receipt["active_row_start_count"], 71)
        self.assertEqual(self.receipt["prime_count"], 29)

    def test_receipt_preserves_boundaries(self):
        self.assertEqual(
            self.receipt["status"],
            "SWEEP_degree5_weak_scale_active_window_phase_curve")
        self.assertTrue(
            self.receipt["finite_weak_scale_phase_curve_sweep_only"])
        self.assertTrue(self.receipt["finite_diagnostic_only"])
        self.assertFalse(self.receipt["goldbach_proved"])
        self.assertFalse(self.receipt["phase_curve_theorem_proved"])
        self.assertFalse(
            self.receipt["robust_margin_universal_theorem_proved"])

    def test_all_swept_rows_dominate_one_half(self):
        self.assertEqual(self.receipt["component_row_count"], 6177)
        self.assertEqual(self.receipt["degree5_total_row_count"], 2059)
        self.assertEqual(self.receipt["dominance_row_count"], 8236)
        self.assertEqual(self.receipt["failure_row_count"], 0)
        self.assertEqual(self.receipt["failure_rows"], [])
        self.assertTrue(
            self.receipt["all_rows_dominate_one_half_signed_full"])
        self.assertEqual(
            self.receipt["all_dominance_slack_sign_counts"],
            {"positive": 8236, "zero": 0, "negative": 0})

    def test_each_prime_summary_passes(self):
        self.assertEqual(len(self.receipt["prime_summaries"]), 29)
        for summary in self.receipt["prime_summaries"]:
            self.assertEqual(summary["active_row_start_count"], 71)
            self.assertEqual(summary["component_row_count"], 213)
            self.assertEqual(summary["degree5_total_row_count"], 71)
            self.assertEqual(summary["dominance_row_count"], 284)
            self.assertTrue(
                summary["all_rows_dominate_one_half_signed_full"])
            self.assertEqual(
                summary["dominance_slack_sign_counts"],
                {"positive": 284, "zero": 0, "negative": 0})

    def test_weakest_weak_scale_phase_curve_row_is_recorded(self):
        weakest = self.receipt["weakest_dominance_row"]
        self.assertEqual(weakest["scale_modulus"], 167)
        self.assertEqual(weakest["prime_modulus"], 181)
        self.assertEqual(weakest["label"], "00,12")
        self.assertEqual(weakest["active_row_start"], 44)
        self.assertAlmostEqual(
            weakest["active_over_full_ratio"],
            0.5370450115638492)
        self.assertAlmostEqual(
            self.receipt["minimum_dominance_slack_above_one_half"],
            0.037045011563849206)


if __name__ == "__main__":
    unittest.main()
