...
class SourceRamanujanTests(unittest.TestCase):
    ...
    def test_source_reduction_reconstructs_leading_lag_mean(self):
        ...
        self.assertIsNotNone(receipt["within_pair_frequency_mass_ratio"])
        self.assertIsNotNone(receipt[
            "across_source_pairs_fixed_frequency_quotient"])
        self.assertAlmostEqual(
            receipt["within_pair_frequency_mass_ratio"]
            * receipt["across_source_pairs_fixed_frequency_quotient"],
            receipt["within_frequency_absolute_mass_ratio"], places=12)
        self.assertAlmostEqual(
            receipt["normalized_pair_frequency_absolute_mass"],
            392973.25954837503, places=6)
        self.assertAlmostEqual(
            receipt["within_pair_frequency_mass_ratio"], 1.0, places=12)
        self.assertAlmostEqual(
            receipt["across_source_pairs_fixed_frequency_quotient"],
            .14869360881703073, places=12)
        self.assertEqual(
            receipt[
                "maximum_across_source_pairs_fixed_frequency_quotient"],
            .25)
        self.assertTrue(receipt[
            "across_source_pairs_fixed_frequency_gate_passes"])
